...
def main():
    ...
    parser.add_argument('-time', default=3600, help="Time delta in sec. Default = 3600 sec", type=int)
    parser.add_argument('-ratio', default=0.85, help="Argument for the graph of the ratio of "
                                                     "targets with an alarm to their total number. Default = 0.85", type=float)
    parser.add_argument('-min_ghost_time', default=1, help="Minimum target lifetime to avoid"
                                                           " becoming a ghost. Default = 1 sec", type=float)
    args = parser.parse_args()

    inp_file_name = args.input_file
    out_file_name = args.output_file
    time_step_sec = args.time
    ...
    for i in range(len(lines_of_file)):
        ...
        if lines_of_file[i].find("Deleted target") != -1:
            arr_string = lines_of_file[i].split()
            if float(arr_string[16]) < args.min_ghost_time:
    ...
    if len(data_for_NewTargets[0]) == 0:
        ...
        first_line_for_output = ["Start time", "N_targets", "Targets_with_alarm", "Ratio"]
        ...
        width_for_plot = 1
        level_of_good_result = args.ratio
        ...
        plt.ylabel("Ratio")
        plt.step(x_axis, DF_out.iloc[:, 2], label="Ratio")
        plt.plot(x_axis, np.ones(len(DF_out)) * level_of_good_result, "r--",
                 label=f"Good result, {level_of_good_result} percent")
        plt.legend()

        DF_out_by_types = DF_out.iloc[:, 4:]

        plt.figure('Log file statistics by types')
        sns.heatmap(DF_out_by_types, annot=True, linewidth=.5)
        plt.xticks(rotation=45)
        plt.show()
